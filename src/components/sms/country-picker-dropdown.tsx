...
const CountryRowItem = React.memo(function CountryRowItem({
  region,
  isSelected,
  onSelect,
}: CountryRowItemProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 min-w-0 flex-1">
        {/* Country Flag with lazy & async decoding */}
        <div className="relative flex items-center justify-center shrink-0 w-5 h-3.5 rounded-[2px] overflow-hidden bg-muted border border-border/50 shadow-2xs">
          {!imgError ? (
            ...
            <span className="w-full h-full flex items-center justify-center text-[8px] font-mono font-bold text-muted-foreground uppercase bg-muted">
              {region.alpha2}
            </span>
          )}
        </div>
...
export function CountryPickerDropdown({
  ...
  // Synchronously reset state on open/close without double-rendering
  ...
  // If search query is present, show matching results immediately (up to 80)
  // If browsing all, progressively render the visible limit for instantaneous opening
  ...
  const handleSelect = React.useCallback((region: RegionRecord) => {
  ...
  return (
    <div className="relative inline-flex items-center">
      <Popover open={open} onOpenChange={handleOpenChange}>
        ...
        {/* Region List with progressive loading on scroll */}
        <div
          onScroll={handleScroll}
          className="overflow-y-auto max-h-64 sm:max-h-72 p-1.5 space-y-0.5 divide-y divide-border/20 custom-scroll"
        >
