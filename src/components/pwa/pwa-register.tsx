...
export function PWARegister() {
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      ...
      setIsOffline(true);
      toast.error('Offline Mode', { description: 'App is running offline using cached assets.' });
    };
  ...
  const triggerInstall = async () => {
    ...
    if (choice.outcome === 'accepted') {
      toast.success(`${appConfig.name} installed successfully!`);
  ...
  return (
    <>
      ...
      {isOffline && (
        ...
        >
          <WifiOff size={14} />
          <span>You are currently offline. Running in PWA offline mode.</span>
        </div>
      )}

      {/* PWA Install Floating Banner / Prompt - Floated Left with Form-Consistent Styling */}
      {installPrompt && (
        <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p style={{ fontSize: '12px', color: 'hsl(var(--muted-foreground))', margin: '2px 0 0 0', lineHeight: '16px' }}>
                  Fast offline access & telematics tracking
                </p>
              </div>
            </div>
            <button
